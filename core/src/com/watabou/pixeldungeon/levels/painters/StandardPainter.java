...
public class StandardPainter extends Painter {
	...
	public static void paint( Level level, Room room ) {
		...
		if (!Dungeon.getInstance().bossLevel() && Random.Int( 5 ) == 0) {
			switch (Random.Int( 6 )) {
			case 0:
			...
			case 5:
				if (!Dungeon.getInstance().bossLevel() && !Dungeon.getInstance().bossLevel( Dungeon.getInstance().depth + 1 ) &&
	...
	private static void paintBridge( Level level, Room room ) {
		...
		fill( level, room.left + 1, room.top + 1, room.width() - 1, room.height() - 1 ,  
			!Dungeon.getInstance().bossLevel() && !Dungeon.getInstance().bossLevel( Dungeon.getInstance().depth + 1 ) && Random.Int( 3 ) == 0 ?
				Terrain.CHASM : 
				Terrain.WATER );

...
public class StandardPainter extends Painter {
	...
	public static void paint( Level level, Room room ) {
		...
		if (!level.dungeon.bossLevel() && Random.Int( 5 ) == 0) {
			switch (Random.Int( 6 )) {
			case 0:
			...
			case 5:
				if (!level.dungeon.bossLevel() && !level.dungeon.bossLevel( level.dungeon.depth + 1 ) &&
	...
	private static void paintGraveyard( Level level, Room room ) {
		...
		for (int i=0; i < nGraves; i++) {
			...
			level.drop( i == index ? Generator.random(level.dungeon) : new Gold(), pos ).type = Heap.Type.TOMB;
		}
	}
	...
	private static void paintBridge( Level level, Room room ) {
		...
		fill( level, room.left + 1, room.top + 1, room.width() - 1, room.height() - 1 ,  
			!level.dungeon.bossLevel() && !level.dungeon.bossLevel( level.dungeon.depth + 1 ) && Random.Int( 3 ) == 0 ?
				Terrain.CHASM : 
				Terrain.WATER );
